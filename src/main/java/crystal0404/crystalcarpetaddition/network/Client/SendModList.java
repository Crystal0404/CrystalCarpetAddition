...
//$$         if (!ClientConfigurationNetworking.canSend(CCANetwork.MOD_LIST)){
...
public class SendModList {
    public static void send_mod(MinecraftClient client, ClientPlayNetworkHandler handler,
                                PacketByteBuf buf, PacketSender sender){
        Collection<ModContainer> get = FabricLoader.getInstance().getAllMods();
        Collection<String> get_mod = new ArrayList<>();
        for (ModContainer modContainer : get) {
        ...
        ModList send_mod = new ModList(get_mod);
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        ClientJoinedTheGame.setMod_list(gson.toJson(send_mod));
    }
}
//#endif
